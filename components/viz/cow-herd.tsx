...
function GradualCow({
  ...
  const isHighlight = highlightIndices ? highlightIndices.includes(index) : index < highlightCount;

  // Map scroll progress → how many cows have finished tinting (0 → highlightCount).
  const fill = useTransform(progress, [0.05, 0.8], [0, highlightCount]);

  // This cow's tint ramps smoothly as fill passes its slot — gradual pink/yellow wash.
  const tint = useTransform(fill, (f) => {
    if (!isHighlight) return 0;
    const slot = highlightIndices ? highlightIndices.indexOf(index) : index;
    const span = Math.max(0.35, 1.2 / Math.max(highlightCount, 1));
    return Math.min(1, Math.max(0, (f - slot) / span));
  });

  const cowColor = useTransform(tint, (t) =>
    t <= 0 ? MUTED : `color-mix(in srgb, ${color} ${Math.round(t * 100)}%, ${MUTED})`,
  );
  const opacity = useTransform(progress, [0, 0.06], [0.35, 1]);
  const scale = useTransform(tint, [0, 1], [0.92, 1]);

  return (
    <motion.span
      className={cn("inline-flex items-end justify-center", cowClassName)}
      style={{ color: cowColor, opacity, scale }}
    >
      <CowGlyph className="h-full w-full" />
    </motion.span>
  );
}

/**
 * CowHerd — cows gradually wash to the accent colour as you scroll (not pop on).
 * Highlighted cows tint pink / yellow / cyan in sequence; the rest stay muted.
 */
export function CowHerd({
  total,
  highlight,
  highlightIndices,
  progress,
  ...
  const highlightCount = highlightIndices?.length ?? highlight ?? 0;
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`${highlightCount} of ${total} cows highlighted`}
    >
